...
describe('background fetch policy', () => {
  it('allows known dictionary and example endpoints', () => {
    ...
    expect(isAllowedBackgroundFetch('dict-fetch', 'http://43.130.246.125:8899/api/word/en/needed')).toBe(true);
  });
  ...
  it('rejects lookalike hosts, credentials, protocols and ports', () => {
    ...
    expect(isAllowedBackgroundFetch('dict-fetch', 'http://43.130.246.125:9999/api/word/en/test')).toBe(false);
  });
});
