...
import { upsertPaddleSubscription } from '../../server/src/paddle.js';
import { hasProAccess, paidAccessGraceMs } from '../../server/src/subscription-access.js';
...
describe('subscription access safety', () => {
  it('trialing only grants access before the explicit trial end', () => {
    expect(hasProAccess({
      status: 'trialing', trialEndsAt: '2026-08-27T00:00:01Z', currentPeriodEndsAt: null,
    }, now)).toBe(true);
    expect(hasProAccess({
      status: 'trialing', trialEndsAt: '2026-08-27T00:00:00Z', currentPeriodEndsAt: null,
    }, now)).toBe(false);
  });

  it('fails closed when a trial has no trustworthy end time', () => {
    expect(hasProAccess({ status: 'trialing', trialEndsAt: null, currentPeriodEndsAt: null }, now)).toBe(false);
  });

  it('gives paid subscriptions a bounded grace period', () => {
    const record = {
      status: 'active', trialEndsAt: null, currentPeriodEndsAt: '2026-08-26T00:00:00Z',
    };
    expect(hasProAccess(record, now, 48 * 60 * 60 * 1000)).toBe(true);
    expect(hasProAccess(record, new Date('2026-08-28T00:00:00Z'), 48 * 60 * 60 * 1000)).toBe(false);
  });

  it('allows past_due only inside the same bounded paid grace period', () => {
    expect(hasProAccess({
      status: 'past_due', trialEndsAt: null, currentPeriodEndsAt: '2026-08-26T12:00:00Z',
    }, now, 24 * 60 * 60 * 1000)).toBe(true);
    expect(hasProAccess({
      status: 'past_due', trialEndsAt: null, currentPeriodEndsAt: '2026-08-25T23:59:59Z',
    }, now, 24 * 60 * 60 * 1000)).toBe(false);
  });

  it('never grants paused or canceled access', () => {
    for (const status of ['paused', 'canceled']) {
      expect(hasProAccess({
        status, trialEndsAt: '2099-01-01T00:00:00Z', currentPeriodEndsAt: '2099-01-01T00:00:00Z',
      }, now)).toBe(false);
    }
  });
...
describe('Paddle subscription cache', () => {
  ...
  it('guards webhook writes against older out-of-order events', async () => {
    ...
    expect(subscriptionQuery[1][14]).toBe('2026-08-20T00:00:00Z');
  });
