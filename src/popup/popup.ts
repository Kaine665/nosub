...
const text = {
  en:{tagline:'Focused listening',on:'NoSub is on',off:'NoSub is paused',onCopy:'Ready for supported YouTube videos',offCopy:'Turn it on when you are ready to practice',repeat:'Focus',previous:'Previous',reveal:'Captions',next:'Next',speed:'Speed',settings:'Open settings',youtube:'Open a YouTube video to start practicing.',reloading:'Reloading this YouTube page…',signedOut:'Sign in to use NoSub Pro',free:'Free account',pro:'Pro active',account:'Sign in / account'},
  zh_CN:{tagline:'YouTube 精听',on:'NoSub 已开启',off:'NoSub 已暂停',onCopy:'已准备好用于支持的 YouTube 视频',offCopy:'准备练习时再将它开启',repeat:'精听',previous:'上一句',reveal:'字幕',next:'下一句',speed:'倍速',settings:'打开设置',youtube:'打开一个 YouTube 视频即可开始练习。',reloading:'正在重新加载 YouTube 页面…',signedOut:'登录后使用 NoSub Pro',free:'免费账号',pro:'Pro 已激活',account:'登录 / 账号'},
} as const;
...
async function init(): Promise<void> {
  ...
  set('tagline',c.tagline); set('repeat',c.repeat); set('previous',c.previous); set('reveal',c.reveal); set('next',c.next); set('speed',c.speed); set('settings-label',c.settings);
