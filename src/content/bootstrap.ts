...
if ((globalThis as { __nosub_bootstrapped?: boolean }).__nosub_bootstrapped) {
  ...
  function trackProductEvent(
    eventName: import('../auth/types.js').ProductAnalyticsEventName,
  ...
  lifecycle.subscribe(async (event) => {
    if (event.type === 'video-detected') {
      runtime.videoSessionId = event.videoSessionId;
      trackProductEvent('youtube_video_opened', event.videoSessionId);
    } else if (event.type === 'session-started') {
      await startController(
        event.session.player,
        event.session.captions,
        event.session.videoId,
        event.session.videoSessionId,
        event.session.captionsMetadataReady,
      );
  ...
  async function startController(
    ...
    if (runtime.videoSessionId !== videoSessionId) return;
    if (!saved.enabled) {
    ...
    controller.subscribeToCoreActions((outcome) => {
      if (runtime.videoSessionId !== videoSessionId) return;
      trackProductEvent('core_action_completed', videoSessionId, {
        action: outcome.action,
        action_result: outcome.actionResult,
        input_method: outcome.inputMethod,
      });
    });

    // 初始化字幕加载
    const result = await controller.init(videoId, captionsMetadataReady);
    if (runtime.videoSessionId !== videoSessionId || runtime.controller !== controller) return;
    if (result.status === 'failed') {
      trackProductEvent('caption_load_failed', videoSessionId, {
        failure_reason: result.failureReason,
      });
      return;
    }
    trackProductEvent('caption_load_succeeded', videoSessionId, {
      caption_language: result.languageCode,
      caption_type: result.isAutoGenerated ? 'automatic' : 'manual',
    });
    trackProductEvent('listening_session_started', videoSessionId);
